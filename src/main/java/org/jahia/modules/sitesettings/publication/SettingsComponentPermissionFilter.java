 ...
 * <p>
 * The site publication screen is an ordinary, instantiable content type, so the settings container it was
 ...
 * {@code j:requiredPermissionNames="publish siteAdminPublication"}), so it is a property of that template and
 * not of the component: the same component rendered through any other resource — a plain page content area —
 * carries no requirement at all, and the web flow behind it would be handed to whoever can render that
 * resource. This filter makes the requirement a property of the component, so it holds on every render path.
 * <p>
